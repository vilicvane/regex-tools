...
import * as RegexTools from '../library';
...
let caseCategoryMap: Dict<Dict<CombineTestCase[]>> = {
  '(?:...) wrapping': {
    ...
    for (let caseName of Object.keys(caseCategory)) {
      it(caseName, () => {
        for (let testCase of caseCategory[caseName]) {
          RegexTools.combine(testCase.regexes).combined.should.equal(
